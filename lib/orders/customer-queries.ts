...
const ORDER_SELECT =
  'id, numero_pedido, status_pagamento, status_producao, valor_total, moeda, criado_em, pago_em, cliente_nome, cliente_email, tipo_atendimento, origem_pagamento'

/** Pedidos visíveis ao cliente: pagos ou dinheiro na entrega/retirada. */
const CUSTOMER_VISIBLE_PAYMENT = ['paid', 'pay_on_delivery'] as const
...
export async function listOrdersForUser(userId: string): Promise<CustomerOrderSummary[]> {
  ...
  const { data, error } = await supabase
    ...
    .in('status_pagamento', [...CUSTOMER_VISIBLE_PAYMENT])
