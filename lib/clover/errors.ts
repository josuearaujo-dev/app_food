...
export function normalizeCloverError(input: {
  ...
  if (status === 401 || status === 403) code = 'configuration_error'
  ...
  else if (/cvv|cvc|security code/.test(raw)) code = 'invalid_cvv'
  else if (/declin|do_not_honor|refused/.test(raw)) code = 'card_declined'
  else if (/invalid.*card|card.*invalid|number/.test(raw)) code = 'invalid_card'
  else if (/duplicate|idempoten/.test(raw)) code = 'duplicate_payment'
